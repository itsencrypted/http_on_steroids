...
import 'package:httponsteroids/src/models/feed.dart';
import 'package:httponsteroids/src/models/item.dart';
import 'package:httponsteroids/src/models/user.dart';

class HttpOnSteroidsClient {
  final Client client;
  final Uri baseUri;

  HttpOnSteroidsClient({Client client, Uri baseUrl})
  ...
  Future<Item> item(int id) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
        '${response.statusCode} Error Fetching HTTP on Steroids Results: ${response.body}',
      );
    }
  }
